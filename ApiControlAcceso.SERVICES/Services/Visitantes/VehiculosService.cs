...
namespace ApiControlAcceso.SERVICES.Services.Visitantes
{
    ...
    {
        #region Dependencies
        private readonly IGenericRepository<VehiculosVisitante> _reporitory;
        private readonly IMapper _mapper;
        private readonly AppDataContext _context;
        #endregion

        #region Constructor
        public VehiculosService(IGenericRepository<VehiculosVisitante> repository, IMapper mapper, AppDataContext context)
        {
        ...
        #region Metodos
        public async Task<bool> InsertOrUpdate(VehiclesDto model)
        {
            try
            {
                if (model != null)
                {
                    var map = _mapper.Map<VehiculosVisitante>(model);
                    if (model.Id == 0)
                    {
